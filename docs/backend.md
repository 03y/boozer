...
| GET    | `/api/v2/items/personalised`                   |                                                                      | get all items with `global_consumption_count` and optional `user_last_consumed` (unix) per row for the logged-in user | requires authentication cookie; sort order is decided by the client |
| GET    | `/api/v2/items/:name`                          | `name` (string) in URL                                               | get a specific item from name; response includes `rating` (float64 average, `0` if none) |                                |
| GET    | `/api/v2/items/:name/leaderboard`              | `name` (string) in URL, (optional) `?time=n`                         | get the top users for an item                                                           |                                |
| GET    | `/api/v2/items/:name/consumptions`             | `name` (string) in URL, (optional) `?time=n`                         | get the number of consumptions for an item                                              |                                |
| GET    | `/api/v2/ratings/:item_id`                     | `item_id` (int) in URL                                               | get the logged-in user's rating for an item                                             | requires authentication cookie; `404` if the user has not rated |
| PUT    | `/api/v2/ratings/:item_id`                     | `item_id` (int) in URL, `rating` (int, 1–5) in body                  | create or update the logged-in user's rating for an item                                | requires authentication cookie; no clear/delete |
...
| GET    | `/api/v2/users/me/leaderboards`                |                                                                      | list private leaderboards you belong to (each entry includes `owner_username`, `invite`) | requires authentication cookie |
...
| GET    | `/api/v2/leaderboards/private/:invite`         |                                                                      | get a private leaderboard (`members`: all members with consumption counts, including zero) |                                |
| POST   | `/api/v2/leaderboards/private/`                |                                                                      | create a private leaderboard                                                            | requires authentication cookie |
| DELETE | `/api/v2/leaderboards/private/:invite`         |                                                                      | delete a private leaderboard (you must be the owner)                                    | requires authentication cookie  |
| POST   | `/api/v2/leaderboards/private/:invite/members` |                                                                      | join a private leaderboard                                                              | requires authentication cookie  |
| DELETE | `/api/v2/leaderboards/private/:invite/members` |                                                                      | leave a private leaderboard                                                             | requires authentication cookie  |
